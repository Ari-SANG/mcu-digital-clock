...
1. Open `firmware/Uart1_Demo.uvproj` in Keil uVision.
2. The student number is configured as `STUDENT_ID_TEXT` in `firmware/config.h`. Its
...
3. Keep the STC clock setting at 11.0592 MHz so the timer and UART calculations
   ...
   baud rate.
4. Build the project. The generated file is `firmware/Objects/Uart1_Demo.hex`.
...
- `firmware/config.h`: student ID, startup time, alarm defaults and timing constants.
...
- `tools/serial-assistant/`: C# WinForms serial tool, source code and compiled EXE.
...
- Immediate screen-off: `07 AA` replies `06` and blanks the display without
  changing or saving the automatic screen-off setting. A button press or a
  ringing alarm wakes it; the clock, alarm and UART keep running.
...
fall back to the defaults in `firmware/config.h`. Older valid EEPROM records are still
...
Run `tools\serial-assistant\bin\SerialTimeSync.exe`, select the CH340 COM port and connect.
...
is also a value to send, not a live readback. Its `立即息屏` button sends a
one-time command independently of that switch.
